// Improved version of your CreateAsync method with better practices

async Task<Guid> CreateAsync(Tenant tenant, Role adminRole, User adminUser, UserRole userRole)
{
    ...
    try
    {
        ...
        // 2. Create Admin Role for the tenant
        ...
        await connection.ExecuteAsync(createRoleSql, adminRole, transaction: transaction);

        // 3. Create Admin User (global user, not tenant-specific)
        ...
        await connection.ExecuteAsync(createUserRoleSql, userRole, transaction: transaction);

        // 5. Create default claims for the admin role
...
async Task CreateDefaultAdminClaimsAsync(IDbConnection connection, IDbTransaction transaction, Guid roleId)
{
    ...
    const string insertClaimSql = @"
        INSERT INTO role_claim (id, role_id, claim_type, claim_value, created_at)
        VALUES (@Id, @RoleId, @ClaimType, @ClaimValue, @CreatedAt)";
