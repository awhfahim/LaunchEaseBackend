...
public class UserRoleRepository : IUserRoleRepository
{
    ...
    public async Task<bool> ExistsAsync(Guid userId, Guid roleId, CancellationToken cancellationToken = default)
    {
        ...
        return await connection.QueryFirstOrDefaultAsync<int?>(sql, new { UserId = userId, RoleId = roleId }) is not null;
    }

    public async Task<IEnumerable<string>> GetRoleNamesForUserAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        using var connection = await _connectionFactory.OpenConnectionAsync();
