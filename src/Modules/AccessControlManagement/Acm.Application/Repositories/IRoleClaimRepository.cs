...
public interface IRoleClaimRepository
{
    ...
    Task<RoleClaim?> GetAsync(Guid roleId, string claimType, string claimValue,
    ...
    Task<bool> ExistsAsync(Guid roleId, string claimType, string claimValue,
        CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<Claim>> GetClaimsForUserRolesAsync(Guid userId, Guid tenantId,
        CancellationToken cancellationToken = default);

    Task DeleteByRoleIdAsync(Guid roleId, DbConnection connection, DbTransaction transaction,
        CancellationToken cancellationToken = default);

    Task AddRangeAsync(List<RoleClaim> claims, DbConnection connection, DbTransaction transaction);
}